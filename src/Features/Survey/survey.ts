...
import {
  ...
  Survey,
  SurveyQuestionQuery,
} from "../types/surveyTypes"; // Đảm bảo import đúng các type
...
export const surveyApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    createSurveyQuestion: builder.mutation({
      query: ({newSurveyQuestion, surveyId}) => ({
        url: `api-surveyQuestion/surveyquestion/${surveyId}`,
        method: "POST",
    ...
    getSurveyQuestions: builder.query<SurveyQuestionQuery[], number>({
      ...
      providesTags: (result, error, surveyId) => [
        { type: "SurveyQuestions", id: surveyId },
      ],
    }),
    // GET LIST SURVEY
